...
- **Responsive UI & Dialogs:** We use a `responsive_layout.dart` wrapper to seamlessly switch between Stack navigation (mobile) and a split-pane layout (desktop/web). To further improve UX on large screens, all bottom sheets (e.g., attachment menus, games menus, activity launchers) have been converted to centered `showDialog` popups. This prevents UI elements from awkwardly anchoring to the bottom corners on wide viewports.
- **Message Grouping (WhatsApp Style):** To make chat threads cleaner, `ChatScreen` implements a dynamic grouping system. Consecutive messages from the same sender within a short timeframe (2 minutes) automatically adjust their `BorderRadius` and margins to visually cluster together, similar to WhatsApp.
...
- **WebRTC Event Sinks (Bad State Errors):** When users refresh the Web App or background the Mobile App, WebRTC connections can drop abruptly. If a `StreamController` is closed during this time, attempting to add new events (like delivery receipts or chat syncs) will throw a `Bad state: Cannot add new events after calling close` error. We mitigated this by ensuring every `.add()` call in `peerdart_service.dart` is strictly guarded by `!streamController.isClosed`. Note that aggressive caching on GitHub Pages can cause old Web App clients to throw this error until a new release tag is pushed.

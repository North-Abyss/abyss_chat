# Abyss Chat Privacy Policy & Terms of Use

**Effective Date:** July 5, 2026

Welcome to Abyss Chat. This application is an open-source, decentralized, peer-to-peer (P2P) messaging and communication platform.

> [!NOTE]
> This document contains our extended terms of use and liability limitations. For a simplified summary of our privacy practices, see the [Basic Privacy Overview](../../PRIVACY.md).

> [!IMPORTANT]
> Your privacy is our top priority. We do not collect, store, or monitor your personal data, messages, or calls. All communications are strictly peer-to-peer.
## 1. Free and Open Source
Abyss Chat is provided 100% free of charge and is open-source software. You are free to inspect, modify, and distribute the code under the terms of its open-source license.

## 2. No Central Servers & Privacy
Your messages, voice calls, video calls, and files are transmitted directly between you and your peers using WebRTC and local network discovery (mDNS), or securely routed via a stateless Cloud Relay.
- **We do not store your data.** 
- **We do not track your activity.**
- **We do not sell any information to third parties.**
- All communication is end-to-end encrypted locally before being transmitted over the network.

## 3. Permissions Explained
To function properly, Abyss Chat requires the following permissions:
- **Camera & Microphone:** Used strictly for peer-to-peer video and audio calling. Your media streams are encrypted and sent directly to your peers. They are never recorded or stored by Abyss Chat.
- **Storage / Files:** Used to let you send files and images to peers, and to store your chat history locally on your own device.
- **Local Network / Internet:** Used to discover peers on your local Wi-Fi via mDNS and to establish WebRTC connections.

## 4. Limitations of Liability (Not at Blame)
Because Abyss Chat operates on a decentralized P2P network, the developers and contributors of Abyss Chat have absolutely no control over:
- The content transmitted through the application.
- The availability or reliability of the network connections.
- Any data loss or security breaches that occur on your local device due to OS vulnerabilities or malicious peers.

**By using Abyss Chat, you agree that the creators and contributors are NOT liable for any damages, losses, or illicit activity conducted over the application.** 

## 5. Usage Rules
- **Stay Safe:** Only connect and share your 6-digit Peer ID with people you trust.
- **Respect Others:** Do not use this application to harass, abuse, or distribute illegal content.

Thank you for using Abyss Chat!

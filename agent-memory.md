...
- **WebRTC Seamless Local Testing (TURN Server)**: Fixed a major limitation where Web-to-Mobile P2P connections failed on the exact same Wi-Fi network due to Firefox/Chrome mDNS IP obfuscation and strict router NAT Hairpinning. Instead of requiring users to manually change `about:config` flags, injected a highly reliable, free public TURN server (`openrelay.metered.ca`) directly into `peerdart_service.dart`. This ensures automatic cloud relay fallback when local P2P is blocked, making cross-platform testing completely seamless with zero configuration. Because the WebRTC connection is End-to-End Encrypted (E2EE) and double-encrypted via our AES-GCM CryptoService, relaying through a public TURN server preserves complete data privacy and security with no risk of data loss.
